...
const FlexConrols = () => {
  ...
  const DirectionButton = (props: { label: string; value: string }) => (
    <Button
      ...
      style={{ fontSize: '2rem' }}
      decoration={
        (!direction && props.value === 'row') || direction === props.value ? 'outline' : 'text'
      }
  ...
  const JustifyButton = (props: { label: string; value: string }) => (
    <Button
      ...
      css={styles.legatureButton}
      decoration={
        (!justifyContent && props.value === 'flex-start') || justifyContent === props.value
          ? 'outline'
          : 'text'
      }
      children={props.label}
  ...
  const AlignButton = (props: { label: string; value: string }) => (
    <Button
      ...
      css={styles.legatureButton}
      decoration={
        (!alignItems && props.value === 'flex-start') || alignItems === props.value
          ? 'outline'
          : 'text'
      }
      children={props.label}
